...
#........................................................................................................................................................................................
            ...
            # --- Unified Classplus/Testbook handler using ITSGOLU API ---
                ...
                api_url_call = f"https://covercel.vercel.app/extract_keys?url={url}@bots_updatee&user_id=7418011150"
                keys_string = ""
                mpd = None
                try:
                    ...
                    # Non-DRM response (direct url)
                        ...
                        await m.reply_text("⚠️Golden Eagle🦅 returned unexpected response, attempting fallback...")
                        # Try helper fallback that used to work for drm-only endpoints
                        try:
                            # Use get_mps_and_keys3 as requested for non-DRM fallback or extraction
                            res = helper.get_mps_and_keys3(url_norm)
                            if res:
                                url = res
                                keys_string = ""
                                print("🔁 Fallback succeeded via helper.get_mps_and_keys3")
                            else:
                                # Try original fallback if 3 fails
                                res2 = helper.get_mps_and_keys2(url_norm)
                                if res2:
                                    mpd, keys = res2
                                    url = mpd
                                    keys_string = " ".join([f"--key {k}" for k in keys])
                    ...
                    await m.reply_text(f"❌https://t.me/rick007contactbot API failed — attempting fallback...")
                    try:
                        res = helper.get_mps_and_keys3(url_norm)
                        if res:
                            url = res
