...
keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton(text="📞 Contact", url=f"tg://openmessage?user_id={OWNER}"), InlineKeyboardButton(text="🛠️ Repo", url="https://t.me/real_extractor")],
        ])      
...
async def start(bot, m: Message):
    ...
    await bot.send_photo(
        chat_id=m.chat.id,
        photo="https://freeimage.host/i/q9PGVHX",
        caption=caption,
...
async def back_to_main_menu(client, callback_query):
    ...
    caption = f"✨ **Welcome [{first_name}](tg://user?id={user_id}) To DRM Uploder**"
...
async def info(bot: Client, update: Message):
    keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(text="𝐁𝐔𝐘 𝐍𝐎𝐖", url=f"tg://openmessage?user_id={OWNER}")]])
    text = (
        f"╭────────────────╮\n"
